...
class FileServerService : Service() {
    ...
    private fun startForegroundServiceNotification() {
        ...
        try {
            ...
            Log.e("FileServerService", "Failed to start foreground", e)
            isRunning = false
            stopSelf()
        }
    }
    ...
    private fun filesResponse(query: String): NioHttpServer.HttpResponse {
        ...
        var first = true
        entries.asSequence()
            .filter { if (showHidden) it.name != "Android" else !it.name.startsWith(".") && it.name != "Android" }
    ...
    private fun qrResponse(query: String, request: NioHttpServer.HttpRequest): NioHttpServer.HttpResponse {
        val path = parseQueryParam(query, "path")
        val name = parseQueryParam(query, "name")
        val isFolder = parseQueryParam(query, "isFolder") == "true"
        
        // Construct the full URL for the client to scan
        val hostHeader = request.getHeader("host", "")
        val baseUrl = if (hostHeader.isNotEmpty()) "http://$hostHeader" else "http://localhost:${SERVER_PORT}"
        ...
        try {
            ...
            val bitMatrix = writer.encode(targetUrl, com.google.zxing.BarcodeFormat.QR_CODE, 256, 256, hints)
